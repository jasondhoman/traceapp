...
import { getArchivedOrders } from '../api/archive';
...
const ArchiveDisplay: React.FC<IDisplay> = ({ reducer }) => {
  ...
  const [columns, setColumns] = useState<typeof defColumns>(
    getLocaleStorageItem('archiveOrderColumns', defColumns, renderCells)
  );
  ...
  const updateColumns = (columnModel: GridColumnVisibilityModel) => {
    ...
    setLocaleStorageItem('archiveOrderColumns', newColumns);
  };
  ...
  return (
    <GridDisplay
      ...
      columnLocalKey="archiveOrderColumns"
    />
  );
};
